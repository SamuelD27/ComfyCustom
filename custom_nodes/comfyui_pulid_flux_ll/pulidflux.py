...
class ApplyPulidFlux:
    ...
    def apply_pulid_flux(self, model, pulid_flux, eva_clip, face_analysis, image, weight, start_at, end_at, attn_mask=None, options={}, unique_id=None):
        ...
        num_double = len(model.model.diffusion_model.double_blocks)
        num_single = len(model.model.diffusion_model.single_blocks)
        ...
        for i in range(num_double):
            if i % pulid_flux.model.double_interval == 0:
                ...
                set_model_dit_patch_replace(model, patch_kwargs, ("double_block", i), is_last_double=(i == last_double_ca))
                ca_idx += 1
